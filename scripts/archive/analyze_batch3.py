"""
ניתוח מלא של Batch 3 (draws/new3/) — 18 שרטוטים.
בודק איכות extraction עם v21 + מדווח על שיפורים וכשלים.
"""
import io
import json
import sys
...
cache_by_md5 = {cf.stem.split("_")[2]: cf
                for cf in Path("output/.cache").glob("v21_*assembly.json")
...
n = len(rows)
print(f"{'='*115}")
print(f"BATCH 3: new3/ ({n} drawings)")
print(f"{'='*115}")
print(f'{"File":<42s} {"P/N":<23s} {"✓":<2s} {"Rev":<5s} {"Customer":<28s} {"CAGE":<6s} {"BOM":<4s} {"Rev":<4s} {"W":<3s} {"$":<7s}')
print("-"*115)
for r in rows:
    cost_s = f"${r['cost']:.3f}"
    print(f'{r["file"]:<42s} {r["pn"]:<23s} {r["pn_match"]:<2s} {r["rev"]:<5s} {r["cust"]:<28s} {r["cage"]:<6s} {r["bom"]:<4} {r["revs_n"]:<4} {r["w"]:<3} {cost_s}')
...
print(f"Total cost:         ${agg['cost']:.3f}  (~₪{agg['cost']*3.7:.2f})")
...
for c, n in customers.most_common():
    print(f"  {n}x  {c}")

if pn_mismatches:
    print()
    print(f"⚠️  P/N לא תואם שם קובץ ({len(pn_mismatches)}):")
    for fname, pn in pn_mismatches:
        print(f"  {fname:<42s} → {pn}")

if sus_vals:
    print()
    print("SUSPICIOUS_STANDARD values:")
